...
def deploy_ambari_hdp(uuid, ip, user="root", password="Letmein123"):
    ...
    httpd_port = install_httpd(logger, ip, user, password).strip()
    print type(httpd_port), httpd_port
    # 上传  ambari 和 hdp 包
    logger.info("start to upload ambari.repo and hdp.repo")
    ambari_repo = os.path.join(os.getcwd(), "ambariInstall\\uploadToLinuxFile\\ambari.repo")
    hdp_repo = os.path.join(os.getcwd(), "ambariInstall\\uploadToLinuxFile\\hdp.repo")
    copy_file_to_linux(ambari_repo, ip, '/home/centos/', user, password)
    copy_file_to_linux(hdp_repo, ip, '/home/centos/', user, password)
    logger.info("upload ambari.repo and hdp.repo complete !")
    ...
    print ambari_url, hdp_url, hdp_utlis_url
    ...
    hdp_path = os.path.join(os.getcwd(), "ambariInstall\\uploadToLinuxFile\\" + hdp_name)
    hdp_util_path = os.path.join(os.getcwd(), "ambariInstall\\uploadToLinuxFile\\" + hdp_util_name)
    ambari_path = os.path.join(os.getcwd(), "ambariInstall\\uploadToLinuxFile\\" + ambari_name)
    ...
    if test_url(ambari_url, 'AMBARI') and test_url(hdp_url, 'hadoop') and test_url(hdp_utlis_url, 'hadoop'):
        write_file(os.path.join(taskdir, 'hdp_status.txt'), 'success')
        logger.info("hdp deploy success")
        return True
    else:
...
def install_httpd(logger, ip, user="root", password="Letmein123", ):
    # 检查是否安装httpd
    logger.info("yum list installed  | grep httpd")
    ...
    if yum_install_httpd.find('httpd') > -1:
        ...
        logger.info("yum -y install httpd")
    ...
    if httpd_status.find('running') > 0:
        ...
        return str(httpd_port)
    if check_linux_port_use(str(httpd_port), ip, user, password):
        # 端口 被占用，修改为 79 端口
        logger.warning(str(httpd_port) + " port is already used ,then update httpd use 79 port ")
...
def test_url(url, keyword):
    ...
    if response.status_code == 200:
        ...
        if text.find(keyword) > 0:
            print "check" + keyword + " success:", url
            return True
    print "check" + keyword + " failed:", url
    return False
